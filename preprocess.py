...
KNN = 20
DESCRIPTOR = 'shot'
MODELS_DIR = '~/MPI-FAUST/training/registrations'
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(MODELS_DIR):
        ...
        for file in files:
            ...
            t = timer()
            _, eigen_x = eigh(laplacian_x,
                                   eigvals=(laplacian_x.shape[0] - NUM_EIGENS, laplacian_x.shape[0] - 1))
            print('eigen function time:', timer() - t)

            t = timer()
            geodesic_x = graph_shortest_path(graph_x_csr,
                                                  directed=False)  # directed=False means geodesic, not l2 distance
